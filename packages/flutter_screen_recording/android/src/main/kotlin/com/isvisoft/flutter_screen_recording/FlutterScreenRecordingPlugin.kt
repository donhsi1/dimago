...
class FlutterScreenRecordingPlugin : 
    ...
    override fun onMethodCall(call: MethodCall, result: Result) {
        ...
        when (call.method) {
            ...
            "stopRecordScreen" -> {
                try {
                    // Stop MediaRecorder + projection first. Unbinding the foreground
                    // service before stopping can leave mMediaRecorder unset / unusable and
                    // causes empty path on Android (VOG/Android 10).
                    val path = if (mMediaRecorder != null) {
                        stopRecordScreen()
                        mFileName ?: ""
                    } else {
                        ""
                    ...
                    println("stopRecordScreen exception: ${e.message}")
                    result.success("")
                }
            }
            else -> {
